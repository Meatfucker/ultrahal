...
class SD15Tab(QWidget):
    def __init__(self, avernus_client: AvernusClient, tabs: VerticalTabWidget):
        ...
        self.resolution_widget = ResolutionInput(placeholder_x="512", placeholder_y="512")
        ...
        self.prompt_layout.addWidget(self.steps_label)
        self.prompt_layout.addWidget(self.batch_size_label)
        self.prompt_layout.addWidget(self.guidance_scale_label)
        self.prompt_layout.addWidget(self.seed_label)
    ...
    async def on_submit(self):
        ...
        try:
            if i2i_image_enable is True:
                request = SD15I2IRequest(avernus_client=self.avernus_client,
            ...
            print(f"SD 1.5 on_submit EXCEPTION: {e}")
...
class SD15Request(BaseImageRequest):
    ...
    async def generate(self):
        ...
        print(f"SD 1.5: {self.prompt}, {self.negative_prompt}, {self.width}, {self.height}, {self.steps}, {self.batch_size}, {self.lora_name}, {self.strength}")
        ...
        try:
            response = await self.avernus_client.sd15_image(self.enhanced_prompt, **kwargs)
            ...
            self.status = "Failed"
            print(f"SD15 REQUEST EXCEPTION: {e}")

class SD15I2IRequest(SD15Request):
